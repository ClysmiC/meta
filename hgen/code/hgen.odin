...
main :: proc()
{
    verify_or_panic(len(os.args) >= 2, "Please provide a target directory argument", scan.Position{});

    dir_arg := canonicalize_dir_string(os.args[1]);
    INCLUDE_SEARCH_ROOT = dir_arg;
    ...
    {
        output_h_str := strings.concatenate([]string{dir_arg, GENERATED_H_FILENAME});
        output_cpp_str := strings.concatenate([]string{dir_arg, GENERATED_CPP_FILENAME});
                ...
                for
                {
                    ...
                    /**/#partial switch token.type
                    {
                        ...
                        */
                        ...
                        case .Template:
                        {
                            match := scan_until_token_set(&scanner, []Token_Type{ .Struct, .Function, .Inline });
                            ...
                            /**/#partial switch match
                            {
                                ...
                                case .Eof:
                                {
                                    verify_or_panic(false, "Expected 'struct', 'function', or 'inline' after 'template<...>'", start);
                                }
                            }
                        }
...
parser_handle_function_keyword :: proc(using hgen: ^Hgen, prev_token: Token, scanner_start: scan.Position, allow_namespace: bool)
{
    ...
    else
    {
        ...
        {
            assert(l_paren == .Eof);
            verify_or_panic(false, "Expected '(' at some point after '' or 'inline'", scanner_start);
        }
        ...
        for
        {
            ...
            for
            {
                ...
                {
                    if token_after_arg.lexeme == "OptionalArg0"
                    {
                        // write type
                        ...
                        verify_or_panic(next_token(&scanner).type == .L_Paren, "Expected '(' after OptionalArg0", scanner_start);

                        name := next_token(&scanner);
                        verify_or_panic(name.type == .Identifier, "Expected identifier after OptionalArg0", scanner_start);

                        // write name
                        write_string(&builder, name.lexeme);

                        verify_or_panic(next_token(&scanner).type == .R_Paren, "Expected ')' after identifier in OptionalArg0", scanner_start);
                    ...
                    else if token_after_arg.lexeme == "OptionalArg"
                    {
                        // write type
                        consume_whitespace(&scanner);
                        write_string(&builder, scanner.src[arg_start : scan.position(&scanner).offset]);
                        ...
                        verify_or_panic(next_token(&scanner).type == .L_Paren, "Expected '('' after OptionalArg", scanner_start);

                        name := next_token(&scanner);
                        verify_or_panic(name.type == .Identifier, "Expected identifier after OptionalArg", scanner_start);

                        // write name
                        write_string(&builder, name.lexeme);

                        verify_or_panic(next_token(&scanner).type == .Comma, "Expected ','' after identifier in OptionalArg", scanner_start);
                        ...
                        verify_or_panic(peek_token(&scanner).type == .R_Paren, "Expected ')' to close OptionalArg", scanner_start);
